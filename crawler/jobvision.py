...
def jobvision():
    ...
    for page_num in range(1,1000):
        ...
        for j in jobs:
            ...
            # Title
            title = j.find("div", class_ = "job-card-title w-100 font-weight-bolder text-black px-0 pl-4").text.strip().replace("\u200c"," ")
            titles.append(title)
            # Link
            ...
            links.append(link)
            # Employer
            employer = j.find("div", class_ = "text-black").text.strip()
            employers.append(employer)
            # Location
            location = j.find("span", class_ = "text-muted ng-star-inserted").text.strip()
            locations.append(location)
            # Tag
            tags.append(None)
            # Description
            inside_jobs = BeautifulSoup(requests.get(link).content, "lxml")
            if inside_jobs.find("div", class_ = "col px-0 mr-2"):
                description = inside_jobs.find("div", class_ = "col px-0 mr-2").text.strip()
            else:
                description = None
...
df = jobvision()
# create csv file
df.to_csv("./bot/crawler/jobvision.csv", index=False)